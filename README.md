...
Add `--cloudflare` to start or reuse a temporary [Cloudflare Quick Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/) and publish an `https://<random>.trycloudflare.com/live/...` URL. `cloudflared` must be installed and available on `PATH`. ReviewLoop probes `PORT`, 5173, and 8787 before starting the tunnel. ReviewLoop uses an isolated Quick Tunnel configuration, selects automatic tunnel transport and checks the public endpoint before printing or reusing a URL. An unreachable tunnel produces an error with its log path instead of a broken review link. The tunnel remains available while its background `cloudflared` process is connected and is reused for later reviews on the same local port.
